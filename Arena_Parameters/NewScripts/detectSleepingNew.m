...
%     %Sometimes there are promblems at the output then the coordinates are
%     %very different. Calculate the  distance
%     d=((cordsAfterExit(:,1)-cordsBeforeEnter(:,1))^2+(cordsAfterExit(:,2)-cordsBeforeEnter(:,2))^2)^0.5;
%     if d<10 %if it is less than 1cm
%     %do average
%     cords=(cordsBeforeEnter+cordsAfterExit)/2;
%     else

        cords=cordsBeforeEnter-2e4;%measure inside the cage
        if isnan(cords)==1 
             cordsBeforeEnter=[num1(SleepingInterval(i,1)-3,2),num1(SleepingInterval(i,1)-3,3)];
   cordsAfterExit=[num1(SleepingInterval(i,2)+3,2),num1(SleepingInterval(i,2)+3,3)];
            cords=(cordsBeforeEnter+cordsAfterExit)/2;
...
raw(1,13)={'Raw velocity'};
  ...
  raw(2:length( CoordT(:,1))+1,14:15)=num2cell( CoordT);
    raw(1,16)={'min dist'};
  raw(2:length(  ResultDist)+1,16)=num2cell(  ResultDist);
  
xlswrite('D:\Test_Tables\TestSilviawithFramesvs9.xlsx',raw)
end
%% Auxiliary functions
