...
% Useful parameters for later use
XYCorners=Params.Coordinates.Corners,
...
%% -----------Define object IsInBridge----------------
IsInBridgeobj=IsInZone;
IsInBridgeobj.LargeBridge=Params.Coordinates.LargerBridge;
IsInBridgeobj.NarrowBridge=Params.Coordinates.NarrowBridge;
%%
...
% According to the choice select fameles or males for working
IndexSexConsider=find(strcmp(Params.Sex,Params.ChoiceList)==1);
miceList=miceList(IndexSexConsider);
Locomotion.AssigRFID.miceList=miceList; %actualize list
...
%        %%-------------------------------- Find is hiding---------------------------------------- 
   ...
   %% ----------------------------Determine when it is stop,running, walking according to the velocity----------------------------  
    ...
    %% -----------------------------------Determine if the mice are in the bridges zone--------------------------
    % Ouput is  IsBridge=[IsLargerBridge,IsNarrowBridge] 2 logical columns.
    % Note: only when the mouse was inside the bridge it was taking into
    % account
    IsBridge=[];
    IsBridge=IsInBridgeobj.IsBridge(Locomotion.AssigRFID.Arena.InArena(:,countMouse),...
                         cell2mat(MouseArray(2:length([CSVFile{2}])+1,3,j)),cell2mat(MouseArray(2:length([CSVFile{2}])+1,4,j)));
    Locomotion.AssigRFID.Arena.IsLargerBridge(:,countMouse)=IsBridge(:,1);
    Locomotion.AssigRFID.Arena.IsNarrowBridge(:,countMouse)=IsBridge(:,2);
    %% Find for each mice which is in the arena,without eating , without cluster the tail and head.
 ...
 if   Params.BodyParams==1 
     ...
     i
     countMouse
     [MajorAxisp,Headp,Tailp]=FindHeadTail(Locomotion,countMouse,XYCorners,max_width); %Output are the head and tail (x,y) for each mouse.
     Locomotion.AssigRFID.Arena.MajorAxis(:,countMouse)= MajorAxisp;
     Locomotion.AssigRFID.Arena.HeadCoordX(:,countMouse)=Headp(:,1);
     Locomotion.AssigRFID.Arena.HeadCoordY(:,countMouse)=Headp(:,2);
     Locomotion.AssigRFID.Arena.TailCoordX(:,countMouse)=Tailp(:,1);  %x and y in pixels units
     Locomotion.AssigRFID.Arena.TailCoordY(:,countMouse)=Tailp(:,2); 
...
%% ------------------------Calculate the events the mouse are near either with head head , tail head or head tail.
if Params.BodyParams==1 
    HeadToHead=[];
    HeadToTail=[];
        [HeadToHead HeadToTail]=PositionBodyCalc(Locomotion,Params.BodyDistance,XYCorners,max_width);
         Locomotion.AssigRFID.Behaviour.HeadToHead=HeadToHead;
       Locomotion.AssigRFID.Behaviour.HeadToTail=HeadToTail;
end
%% -------------------------------Calculating chasing-----------------------------
...
%% 
% %% %%                                                  CALCULATION OF
% APPROACHING (mouse 1 approaches mouse 2 and the event finished)including
% Avoidance
  [approaching,beavoiding]=Approaching(Params,Locomotion)

% Save the variables
Locomotion.AssigRFID.Behaviour.Approaching=approaching;
Locomotion.AssigRFID.Behaviour.BeAvoiding=beavoiding;

hhhh=msgbox('Approaching and Avoidance calculation was finished')
close(hhhh)
...
%% %% ---------------------------------Arrange the chasing events/approaching and avoiding events into an excel file AGREGAR LO QUE FALTA----------------------------

[AllChasing AllApproaching  AllBeAvoiding]=GetEvents;
%% --------------------SAVE SOCIAL EVENTS INTO EXCEL FILE-----------------
      ...
      %for chasing
        if ~isempty(AllChasing)
          SaveSocialData(AllChasing,Params.SaveDirectory,Params.exp_name,Params)
        end
        %for approaching
          if ~isempty(AllApproaching)
             SaveSocialDataApproaching(AllApproaching,Params.SaveDirectory,Params.exp_name,Params)
          end
        %for be avoiding
        if ~isempty( AllBeAvoiding)
          SaveSocialDataAvoiding(AllBeAvoiding,Params.SaveDirectory,Params.exp_name,Params)
        end
...
function SaveSocialData(Social_Data,SaveDirectory,exp_name,Params)
...
function SaveSocialDataAvoiding(Social_Data,SaveDirectory,exp_name,Params)
...
function SaveSocialDataApproaching(Social_Data,SaveDirectory,exp_name,Params)
