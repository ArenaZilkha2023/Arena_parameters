...
Params.BodyParams=get(h.CheckboxBodyParams,'Value');
...
S=load(strcat(FolderParams,'MovingParametersInArena.mat'));
HidingCoordinatesCentral=S.HidingCoordinatesCentral;
XYCorners=S.Corners_PixelCoordinates;
...
%%   
 ...
 CoordSleepingCells=SleepingBox;
BridgesCoordinatesNarrow=S.NarrowBridge_PixelCoordinates;
BridgesCoordinatesLarger=S.LargeBridge_PixelCoordinates;


%% ---------------Conversion to mm--------------------------------
HidingCoordinates=[S.First_HidingCoordinates;S.Second_HidingCoordinates;S.Third_HidingCoordinates;S.Four_HidingCoordinates];
ZoneData=S.OutZone_PixelCoordinates; %this is given by a vertix , width and heigth
ZoneCoord=[ZoneData(1,1) ZoneData(1,2);(ZoneData(1,1)+ZoneData(1,3)) ZoneData(1,2);(ZoneData(1,1)+ZoneData(1,3)) (ZoneData(1,2)+ZoneData(1,4));ZoneData(1,1) (ZoneData(1,2)+ZoneData(1,4))];

% % pixels--> mm rescaling based on max_wd (cm) and XYCorners (pix-ref) data
 HidingCoordinates(:,1)=PixelsToMM(HidingCoordinates(:,1),XYCorners(1,1),XYCorners(2,1),max_width);
 HidingCoordinates(:,2)=PixelsToMM(HidingCoordinates(:,2),XYCorners(1,2),XYCorners(4,2),max_width);
 
 FoodCoordinates(:,1)=PixelsToMM(FoodCoordinates(:,1),XYCorners(1,1),XYCorners(2,1),max_width);
 FoodCoordinates(:,2)=PixelsToMM(FoodCoordinates(:,2),XYCorners(1,2),XYCorners(4,2),max_width);
 
 WaterCoordinates(:,1)=PixelsToMM(WaterCoordinates(:,1),XYCorners(1,1),XYCorners(2,1),max_width);
 WaterCoordinates(:,2)=PixelsToMM(WaterCoordinates(:,2),XYCorners(1,2),XYCorners(4,2),max_width);
 
 CoordSleepingCells(:,1)=PixelsToMM(CoordSleepingCells(:,1),XYCorners(1,1),XYCorners(2,1),max_width);
 CoordSleepingCells(:,2)=PixelsToMM(CoordSleepingCells(:,2),XYCorners(1,2),XYCorners(4,2),max_width);
 
 %HidingCoordinates=PixelsToMM(HidingCoordinates,XYCorners,max_width);
 
 ZoneCoord(:,1)=PixelsToMM(ZoneCoord(:,1),XYCorners(1,1),XYCorners(2,1),max_width);
 ZoneCoord(:,2)=PixelsToMM(ZoneCoord(:,2),XYCorners(1,2),XYCorners(4,2),max_width);
 
 BridgesCoordinatesNarrow(:,1)=PixelsToMM(BridgesCoordinatesNarrow(:,1),XYCorners(1,1),XYCorners(2,1),max_width);
 BridgesCoordinatesNarrow(:,2)=PixelsToMM(BridgesCoordinatesNarrow(:,2),XYCorners(1,2),XYCorners(4,2),max_width);
 
 BridgesCoordinatesLarger(:,1)=PixelsToMM(BridgesCoordinatesLarger(:,1),XYCorners(1,1),XYCorners(2,1),max_width);
 BridgesCoordinatesLarger(:,2)=PixelsToMM(BridgesCoordinatesLarger(:,2),XYCorners(1,2),XYCorners(4,2),max_width);
%% 
% ----------------------------Save settings in a structure called Params----------------------------- 
 Params.Coordinates.HidingCoordinates=HidingCoordinates; %give you the coordinates of each hiding box from top left in clockwise
 Params.Coordinates.FoodCoordinates=FoodCoordinates; % In mm
Params.Coordinates.WaterCoordinates=WaterCoordinates;
 Params.Coordinates.CoordSleepingCells=CoordSleepingCells;
 Params.Coordinates.XYCorners=XYCorners;
%Params.HidingCoordinatesCentral=HidingCoordinates; 
Params.ZoneCoord=ZoneCoord; 
Params.Coordinates.NarrowBridge=BridgesCoordinatesNarrow; % in mm
Params.Coordinates.LargerBridge=BridgesCoordinatesLarger; % in mm

Params.Coordinates.Corners=XYCorners; % in pixels
...
% Params.editpar6=str2num(get(h.editpar6,'string'));
 ...
 Params.TreshVelocity=100; % threshold velocity was 150cm/sec no more than this velocity is allow
...
Params.MaximumDistanceToApproach=str2num(get(h.Approaching1,'string')); %distance in mm
Params.ApproachingAngle=str2num(get(h.Approaching4,'string'));
Params.Aproaching_velocity= str2num(get(h.Approaching3,'string'));
Params.VelocityApproachedMouse=str2num(get(h.Approaching2,'string'));

%% Parameter of body position (either head head head tail)
if Params.BodyParams==1
 Params.BodyDistance=str2num(get( h.bodyDist,'string'));
end
...
if Params.exp_name(end)=='L'
  ...
  Params.SleepingBox=[Sbox1;Sbox2;Sbox3;Sbox4;Sbox5;Sbox6;Sbox7;Sbox8];
end  
 
if Params.exp_name(end)=='R'
  %% Insert coordinates of sleeping box in mm
  %Do 5 mm from the border
  Sbox8=[5,269.7]; %this is related to antenna 8 and 56
  Sbox7=[5,869.1]; %this is related to antenna 43 and 55
 ...
 %% Load information about sex
 sheet=Params.exp_name;
 [num txt raws]=xlsread(strcat(FolderParams,'MiceID.xlsx'),sheet);
IndexSex=find(strcmp(raws(1,:),'Sex')==1);
Params.Sex=strrep(raws(2:end,IndexSex),'''','');
