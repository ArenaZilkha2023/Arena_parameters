function [approaching beavoiding]=Approaching(Params,Locomotion)
% Method according Silvia
  ...
  approaching=cell(1,numOfMice);
  beavoiding=cell(1,numOfMice);
          ...
          %% 
             ...
             Trajectory2(~isInArena(:,mouse2)|(TrajectoryX(:,mouse2)==1e6),:)=0; % if it is outside the arena and the trayectory is non-defined
                 if sum(sum(Trajectory2))==0 %in the case is not in the arena
                       ...
                       % 1-First condition the 2 mice at the arena and
                       % defined position
...
% %                        % Get the events
                            ...
                            % If approaching 
                                if ~isempty(BeginApproaching)
                                   [Approaching1To2 BeAvoiding1To2]=IsApproaching(mouse2,BeginApproaching,EndApproaching,v1,v2,Distance,angle1,angle2,Params.ApproachingAngle,Params.Aproaching_velocity,Params.VelocityApproachedMouse,...
                                                                     Params.Coordinates.FoodCoordinates,Locomotion.AssigRFID.XcoordMM,Locomotion.AssigRFID.YcoordMM,mouse1);% Add coordinates and food to eliminate the events on the food stand
                                   approaching{mouse1}=[approaching{mouse1};num2cell(Approaching1To2)];
                                   beavoiding{mouse1}=[beavoiding{mouse1};num2cell(BeAvoiding1To2)];
                                end
...
result=acos(coseno)*180/pi;% in degrees units
end
end
%% 
...
result=acos(coseno)*180/pi; % angle in degrees
else
    result=1000;
end
end
