function  MainGUI_ValidationSocialEvents(~,~)
%variables
global h
global Image2
global Image3
...
% %% --------------------------------Add checkbox-----------------------------------
%   h.checkboxVideoGV=uicontrol(h.FigVideo,'Style', 'checkbox','units','pixels',.....
%     'string','Coordinates from GV (2013)','Value',0,...
%     'position',[700,70,250,10],'FontSize',9,'ForegroundColor','blue');
% 
%  h.checkboxVideoNew=uicontrol(h.FigVideo,'Style', 'checkbox','units','pixels',.....
%     'string','Coordinates from new (2017)','Value',1,...
%     'position',[700,40,250,10],'FontSize',9,'ForegroundColor','blue');

%% ---Create button to create a movie----------
      h.CreateMovie=uicontrol(h.FigVideo,'Style', 'pushbutton','units','pixels',.....
    'tag','Directory',...
    'callback',@CreateMovie,'cdata',Image7,...
    'position',[900,200,40,60],'Visible','on');
   uicontrol(h.FigVideo,'Style', 'text','units','pixels',.....
    'string','Create movie with the events',...
    'position',[850,170,150,30],'FontSize',9,'ForegroundColor','blue');

%----------BUTTON TO WORK WITH THE MOVIES-------------
  h.ReadMovie=uicontrol(h.FigVideo,'Style', 'pushbutton','units','pixels',.....
    'tag','Directory',...
    'callback',@MainGUI_ValidationSocialEvents_ReadMovie,...
    'position',[850,150,150,30],'Visible','on','string','Use movies to find the events',...
    'FontSize',9,'ForegroundColor','red','Background','blue','FontSize',8);
...
%-------------------for editing the number  of event------------------------
    ...
    'string','1',...
    'position',[860,830,60,40],'FontSize',9,'ForegroundColor','blue');

     uicontrol(h.FigVideo,'Style', 'text','units','pixels',.....
    'string','Number of social events',...
    'position',[820,870,150,20],'FontSize',9,'ForegroundColor','blue');

%% ---------------------------------Add number of frames before and after to add-----------------------
      h.editNumberFrames=uicontrol(h.FigVideo,'Style', 'edit','units','pixels',.....
    'string','0',...
    'position',[200,40,50,30],'FontSize',9,'ForegroundColor','blue');

    uicontrol(h.FigVideo,'Style', 'text','units','pixels',.....
    'string','Number of frames before and after a given event',...
    'position',[150,10,150,30],'FontSize',9,'ForegroundColor','blue');


%-------------------------------------Add a slider to the-------
   h.slider=uicontrol(h.FigVideo,'Style', 'slider','units','pixels',.....
    'position',[100,90,750,20],'FontSize',9,'ForegroundColor','blue','callback',@MoveFrames);
%    hLstn = handle.listener(h.slider,'ActionEvent',@MoveFrames);
  %---------------------------------Add experiment name---------------------- 
   h.editExpName=uicontrol(h.FigVideo,'Style', 'edit','units','pixels',.....
    'string','Exp51L',...
    'position',[660,795,60,20],'FontSize',9,'ForegroundColor','red');

    uicontrol(h.FigVideo,'Style', 'text','units','pixels',.....
    'string','Number of experiment',...
    'position',[630,820,150,20],'FontSize',9,'ForegroundColor','red');
%% --------Add button to correct your events-----------------
  ...
  h.PopupCorrectionsEvents = uicontrol('Style', 'popup',...
   ...
   h.okStartButton=uicontrol('Style', 'pushbutton','units','pixels',.....
    'tag','Directory',...
    'callback',@SaveCorrectChasing,'cdata',Image5,...
    'position',[900,660,20,30]);

%% ----------------------Create button for selecting social event---------------------
 uicontrol(h.FigVideo,'Style', 'text','units','pixels',.....
    'string','Select social parameter',...
    'position',[620,880,200,20],'FontSize',9,'ForegroundColor','magenta');

  % Create pop-up menu with the social events
  h.SelectSocialParameter = uicontrol('Style', 'popup',...
           'String', {'Chasing','Avoiding','Approaching',''},...
           'Position', [660,860,100,20]);
...
%--------------------Create panel for video------------------------
 ...
 global h
   ...
   uicontrol(Figure_handles,'Style', 'text','units','pixels',.....
    'string','2-Open the folder with Video and Video avi',...
