function Array =  ShufflingGetPvalueGlicko(EloeTotal,IndexOrder,TimeCalculation,MouseID,numR)

%% Create array for pvalues
Array={};
Array(2:length(MouseID)+1,1)=strcat('''',MouseID(IndexOrder),'''');%arrange according to dominant
Array(1,2:length(MouseID)+1)=strcat('''',MouseID(IndexOrder),'''');

numR = cell2mat(numR);
%% 

%IN THE CASE OF 5 MICE according to last glicko ranking
