function SaveArray=RunShuffling(DataToUse,FilenameN)
...
ShufflingNumberTimes=Params.ShufflingNumberTimes;
...
for i=1:length(I)
   I1=find( strcmp(strrep(NewSubsetData(i,IndexChasing),'''',''),MouseID)==1);%found mouse number
   I2=randperm(5,1);
    while I2==I1
        I2=randperm(5,1);
    end
   NewSubsetData(i,IndexBeingChasing)=MouseID(I2); 
...
eloe  = EloRatingCalculation( NewSubsetData,MouseID,k,initial_rating,IndexChasing,IndexBeingChasing,IndexEvents );
eloe=eloe';
...
Delta=EloeTotal(size(EloeTotal,1),DominantMouse,:)-EloeTotal(size(EloeTotal,1),SubmisiveMouse,:);
...
%for last event
 TimeCalculation=size(EloeTotal,1);
 
 PvalueArrayLastEvent=ShufflingGetPvalue(EloeTotal,IndexOrder,TimeCalculation,MouseID,numR); % get p value from the differences. from more dominant to less dominant mouse
 PvalueArrayLastEvent(1,1)={'Last Event'};
 %% Do the same Per day
%Arrange num elo according to the ranking of the last event

for i=1:length(MouseID)
   numElo1(:,i)=numElo(:,IndexOrder(i)); 
    
end

 for i=2:size(numElo,1)
%  dataexp=[];
 dataexp=numElo1(i,:);
 TimeCalculation=i;
 try
 PvalueArrayDay(:,:,i-1)=ShufflingGetPvalue(EloeDayTotal,IndexOrder,TimeCalculation,MouseID,dataexp');
 catch
     
   msgbox('Warning: CHECK THAT YOU ARE NOT SAVING ON ANOTHER EXPERIMENTS DAYS-DO AGAIN!!!') 
     
 end
 PvalueArrayDay(1,1,i-1)=cellstr(strcat('Day',num2str(i-1)));
...
SaveArray={};
SaveArray=[PvalueArrayLastEvent;cell(1,size(PvalueArrayLastEvent,2))];
for i=1:size(PvalueArrayDay,3)
SaveArray=[SaveArray;PvalueArrayDay(:,:,i);cell(1,size(PvalueArrayDay,2))];
end

DataToUse.SaveArray=SaveArray;
...
hh=msgbox('The running was finished')
close(hh)
end
