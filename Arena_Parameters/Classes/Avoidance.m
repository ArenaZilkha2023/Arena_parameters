classdef Avoidance
    ...
    methods
        ...
        function AvoidanceCal=AvoidanceCal(obj,TrajectoryX,TrajectoryY,Velocity,isInArena)
                           ...
                           if isempty(find(isInArena(:,mouse2)==1))%in the case all the arena is zero and not mouse in the arena
                               ...
                               avoiding1of2=detectAvoidance(Trajectory1,Trajectory2,Velocity(:,mouse1),Velocity(:,mouse2),Distance_thresh,Dist_thresh_Fine_Tuning,velThr,R);  
                                  if ~isempty(avoiding1of2)
                                     avoiding1of2=[avoiding1of2(:,1) avoiding1of2(:,2) mouse2*ones(size(avoiding1of2,1),1)];%the first intervals correspond to without repeats 
...
flag=false;
i=find(distance<Dist_thresh_Fine_Tuning,1,'last'); %find the index before it is open the avoiding with less velocity than being avoiding
...
flag_angle=Flag_angle(trajectory1(:,:),trajectory2(:,:));
...
if flag_angle==1
    if ~isempty(i)
        if i<n-2&&i>1 %Avoid situation that i is the beggining or end of the interval
          if sum(velocity1(i+1:n)>velTr1)>=(n-i)/2 %mouse1 running away
            T=trajectory1(i,:);
