...
%%                         
             ...
             %------Small events
             ChasingVector=FillGap(BegChasingCorrected1, EndChasingCorrected1,ChasingVector,Trajectory1,Trajectory2,obj.GapPath,obj.GapFrames);
             [BegChasingCorrected3 EndChasingCorrected3]=getEventsIndexesGV(ChasingVector,length(ChasingVector)); 
             
             %----Large events-----------------
              ChasingVectorWithSmallEvents=FillGapSmall(BegChasingCorrected1S, EndChasingCorrected1S,ChasingVectorWithSmallEvents,Trajectory1,Trajectory2,obj.GapPath,obj.GapFrames);
             [BegChasingCorrected3S EndChasingCorrected3S]=getEventsIndexesGV(ChasingVectorWithSmallEvents,length(ChasingVectorWithSmallEvents)); 
...
%              [BegChasingCorrected3 EndChasingCorrected3]=getEventsIndexesGV(ChasingVector,length(ChasingVector)); 
            ...
            ChasingVectorWithSmallEvents=RealEvents(BegChasingCorrected4S, EndChasingCorrected4S,ChasingVectorWithSmallEvents,DeltaX(:,mouse1),DeltaY(:,mouse1),DeltaX(:,mouse2),DeltaY(:,mouse2),obj.MinimumPath);
...
function ChasingVector=FillGapSmall(BegChasing, EndChasing,ChasingVector,Trajectory1,Trajectory2,GapPathEnd,GapFrames)

if ~isempty(BegChasing)& length(BegChasing)>1
 for i=1:length(BegChasing)-1
    if (BegChasing(i+1)-EndChasing(i))<GapFrames/5 %if the difference in frames between gaps in the chasing is less than a given value REDUCE TEN TIMES THE GAP
     %check if the trajectories are near
...
if ~isempty(BegChasing)
   ...
   for i=1:length(BegChasing)
    ...
    if DistFun1< 2*MinimumPath | DistFun2< 2*MinimumPath
  ...
  for i=1:length(EventsBegCD)
    if EventsBegCD(i)>1 & EventsEndCD(i)<length(ChasingVector) %assure that it is a middle event not in the extrem
      ...
      if Alpha<0 %the event is not correct
        ...
        ChasingVector(EventsBegCD:EventsEndCD)=0;  
